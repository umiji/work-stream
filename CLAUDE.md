...
ユーザーへの回答・説明は、コード内のシンボル名や変数名だけを羅列した技術的すぎる言い方を避け、**ユーザー（人間）の視点に立ったわかりやすい説明**を優先する。

- **関数名や変数名だけの羅列を避ける**: 「`ensureGaConfigured` 内の `isDev` が `false` になる」のような説明だけでは、受け取る側が「それが何を指す機能なのか」を直感的に理解できない
